...
// Agressive Cows  spoj
// ussi binary search monotonic vale concept pe hoga 
// har baar check karege ki kitni cows aa sakti ha usse basis pe binary search lagayege
	public static int bs(int[] arr, int k){
		Arrays.sort(arr);   // sort karna ha 
		int si = 1, ei = 1000000000;
		
		while(si <= ei){
			int mid = ei + (si-ei)/2;
			
			int cowplaced = solve(arr, mid);
			if(cowplaced >= k) si = mid+1;
			else ei = mid-1;
		}
		return ei;   // (ei) hi ayega 
	}
	
	public static int solve(int[] arr, int distance){
		int n = arr.length;
		int countcow = 0, diff = 0, lastplacedcow = arr[0]-distance;
		
		for(int i = 0; i < n; i++){
			diff = Math.abs(arr[i]-lastplacedcow);
...
// 1552. Magnetic Force Between Two Balls -> ye question pura aggresive cows ka hi ha code bhi ditto same ha
//============================